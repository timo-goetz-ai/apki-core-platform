...
function ServiceCard({ item }: { item: typeof DB_CARDS[number] | typeof STORAGE_CARDS[number] }) {
  const cardContent = (
    <div style={{
      ...
      onMouseEnter={e => { if (item.link) e.currentTarget.style.borderColor = 'var(--accent-blue, #3b82f6)'; }}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--text-primary, #f1f5f9)' }}>{item.name}</p>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--text-secondary, #64748b)' }}>{item.description}</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <StatusDot online={item.status === 'online'} />
          <span style={{ fontSize: 11, color: '#22c55e' }}>Online</span>
          {item.link && <ExternalLink size={12} color="var(--text-secondary, #64748b)" />}
        </div>
      </div>
      <div>
        <span style={{
    ...
    <div style={{ padding: '24px 32px', minHeight: '100vh', fontFamily: 'var(--font-ui, inherit)' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--text-primary, #f1f5f9)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <Database size={22} color="var(--accent-blue, #3b82f6)" />
          Datenbanken &amp; Storage
        </h1>
      ...
      <section style={{ marginBottom: 32 }}>
        <p style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary, #64748b)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Datenbanken
        </p>
      ...
      <section>
        ...
        <div style={{
          ...
          <div style={{
            ...
            padding: '8px 18px', borderBottom: '1px solid var(--border, #2d3748)',
          }}>
            {['Name', 'Base ID', 'Tabellen', ''].map((h, i) => (
              <span key={i} style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary, #64748b)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{h}</span>
            ))}
          </div>
          {NOCODB_BASES.map((base, i) => (
            <div
              ...
              style={{
                ...
                alignItems: 'center',
                padding: '12px 18px',
                borderBottom: i < NOCODB_BASES.length - 1 ? '1px solid var(--border, #2d3748)' : 'none',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <StatusDot online />
                <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary, #f1f5f9)' }}>{base.name}</span>
              </div>
              <span style={{ fontSize: 12, fontFamily: 'var(--font-mono, monospace)', color: 'var(--text-secondary, #64748b)' }}>
                {base.id}
              </span>
              ...
              <a
                ...
                onMouseLeave={e => (e.currentTarget.style.background = 'var(--layer-3, #2a3347)')}
              >
